...
def p_dec_Arr_1D(p):
    '''
    dec_Arr : ID LCAS expression_arr RCAS DOUBLEPOINT type SEMICOLON
    '''
    ...
    global base
    simbolo = p[1]  # Obtenemos el simbolo
    tipo = p[6]  # Obtenemos el tipo de la variable
    dimension1 = pila_operandos.pop()
    dimension2 = 1
    dimension3 = 1
    ...
    if simbolo in tabla_de_simbolos:
        ...
        # Tabla de simbolos
        #Simbol_Index = LittleTools.updateSimbolTable(simbolo,tipo,tabla_de_simbolos,Simbol_Index)
        # Creamos espacios de memoria la variable tipo arreglo
        LittleTools.createSpaceInMemory(simbolo,dimension1,dimension2,dimension3,memArreglos)
        LittleTools.updateSimbolTable_ARR_1D(simbolo,tipo,tabla_de_simbolos,Simbol_Index,base,dimension1,dimension2,dimension3)
...
def p_dec_Arr_2D(p):
    '''
    dec_Arr : ID LCAS expression_arr RCAS LCAS expression_arr RCAS DOUBLEPOINT type SEMICOLON
    '''
                                                          # NOMBRE_VAR DIMENSION : INT/FLOAT/STRING ; Aun no se inicializan con un valor
    #    ACTUALIZACION TABLA DE SIMBOLOS  #
    global Simbol_Index # Nos permite utilizar la variable global dentro de la actualizacion de simbolos
    ...
    simbolo = p[1]  # Obtenemos el simbolo
    tipo = p[9]  # Obtenemos el tipo de la variable
    if simbolo in tabla_de_simbolos:
        compilador.exit(f"{simbolo} Array Already declared!")
    else:
        # Tabla de simbolos
        dimension2 = pila_operandos.pop()
        dimension1 = pila_operandos.pop()
        dimension3 = 1
        #Simbol_Index = LittleTools.updateSimbolTable(simbolo,tipo,tabla_de_simbolos,Simbol_Index)
        LittleTools.createSpaceInMemory(simbolo,dimension1,dimension2,dimension3,memArreglos)
        LittleTools.updateSimbolTable_ARR_2D(simbolo,tipo,tabla_de_simbolos,Simbol_Index,base,dimension1,dimension2,dimension3)
...
def p_dec_Arr_3D(p):
    '''
    dec_Arr : ID LCAS expression_arr RCAS LCAS expression_arr RCAS LCAS expression_arr RCAS DOUBLEPOINT type SEMICOLON
    '''
                                                          # NOMBRE_VAR DIMENSION : INT/FLOAT/STRING ; Aun no se inicializan con un valor
    #    ACTUALIZACION TABLA DE SIMBOLOS  #
    global Simbol_Index # Nos permite utilizar la variable global dentro de la actualizacion de simbolos
    global base
    simbolo = p[1]  # Obtenemos el simbolo
    tipo = p[12]  # Obtenemos el tipo de la variable
    if simbolo in tabla_de_simbolos:
        compilador.exit(f"{simbolo} Array Already declared!")
    else:
        # Tabla de simbolos
        ...
        # Cuadruplos
        dimension3 = pila_operandos.pop()
        dimension2 = pila_operandos.pop()
        dimension1 = pila_operandos.pop()
        #LittleTools.genCuadruplos(simbolo,"=",pila_operandos)
        LittleTools.createSpaceInMemory(simbolo,dimension1,dimension2,dimension3,memArreglos)
        LittleTools.updateSimbolTable_ARR_3D(simbolo,tipo,tabla_de_simbolos,Simbol_Index,base,dimension1,dimension2,dimension3)

        
#   ACTUALIZACION DE VARIABLES TIPO ARREGLO     #
def p_act_Arr_1D(p):
    '''
    act_Arr : ID LCAS expression_arr RCAS ASSIGN expression SEMICOLON
    '''
...
def p_act_Arr_2D(p):
    '''
    act_Arr : ID LCAS expression_arr RCAS LCAS expression_arr RCAS ASSIGN expression SEMICOLON
    '''
                                                          # NOMBRE_VAR DIMENSION = V_int/V_string/V_float/variable ;
    simbolo = p[1] # Obtenemos el simbolo
    valor = p[4]
...
def p_act_Arr_3D(p):
    '''
    act_Arr : ID LCAS expression_arr RCAS LCAS expression_arr RCAS LCAS expression_arr RCAS ASSIGN expression SEMICOLON
    '''
                                                          # NOMBRE_VAR DIMENSION = V_int/V_string/V_float/variable ;
    simbolo = p[1] # Obtenemos el simbolo
    valor = p[4]
...
for key in tabla_de_simbolos:
    print(key, ' : ', tabla_de_simbolos[key])  
print("-------------")
for x in range (len(memArreglos)):
    print("|INDEX | Value|")
    print("|",x,"  |",memArreglos[x],"  |") 
print("-------------")
